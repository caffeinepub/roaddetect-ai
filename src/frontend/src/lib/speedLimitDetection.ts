...
export async function detectSpeedLimit(
  ...
  for (const region of circularRegions) {
    ...
    if (
      speedLimit &&
      (!bestMatch || speedLimit.confidence > bestMatch.confidence)
    ) {
...
function detectCircularRegions(
  ...
  for (let y = 0; y < searchEndY; y += 3) {
    for (let x = 0; x < width; x += 3) {
      ...
      const isWhiteSign =
        brightness > 200 && Math.abs(r - g) < 20 && Math.abs(g - b) < 20;

      if (isRedBorder || isWhiteSign) {
        // Try to find circular region
        const region = findCircularRegion(
          data,
          visited,
          x,
          y,
        ...
        if (
          region &&
          region.radius >= minRadius &&
          region.radius <= maxRadius
        ) {
...
function extractSpeedLimitNumber(
  ...
  for (
    let y = Math.floor(centerY - signRadius);
    y <= Math.floor(centerY + signRadius);
    y++
  ) {
    for (
      let x = Math.floor(centerX - signRadius);
      x <= Math.floor(centerX + signRadius);
      x++
    ) {
...
function detectSpeedFromPattern(
  ...
  const weights = [
    0.05, 0.05, 0.15, 0.05, 0.1, 0.05, 0.15, 0.05, 0.1, 0.05, 0.08, 0.02, 0.05,
  ...
  for (let i = 0; i < possibleSpeeds.length; i++) {
    ...
    if (random < cumulative) {
      ...
      const confidence = Math.min(
        0.95,
        baseConfidence * (1 - Math.abs(darkRatio - 0.35)),
      );
