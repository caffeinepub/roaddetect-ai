...
export default function AccidentHistoryList({
  onSelectAccident,
}: AccidentHistoryListProps) {
    ...
    return (
      <div className="rounded-lg border-2 border-destructive bg-destructive/10 p-6 text-center">
        ...
        <p className="text-sm text-destructive">
          Failed to load accident records
        </p>
      </div>
    );
  }
  ...
  if (accidents.length === 0) {
    return (
      <div
        data-ocid="accident_reports.empty_state"
        className="rounded-lg border-2 border-dashed border-primary/20 bg-primary/5 p-12 text-center"
      >
        <AlertTriangle className="mx-auto mb-4 h-12 w-12 text-primary opacity-40" />
        <h3 className="mb-2 text-lg font-semibold">No Accident Reports Yet</h3>
        <p className="text-sm text-muted-foreground">
          No accidents have been reported yet. Use the{" "}
          <span className="text-primary font-medium">Report Accident</span>{" "}
          button above to create a new report.
        </p>
      </div>
    );
  }
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {accidents.map((accident, idx) => {
        ...
        return (
          <Card
            key={accident.id}
            data-ocid={`accident_reports.item.${idx + 1}`}
