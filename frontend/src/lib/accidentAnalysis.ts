...
export async function analyzeAccidentScene(imageData: ImageData): Promise<AccidentAnalysisResult> {
  ...
  const averageConfidence = indicators.length > 0 ? totalConfidence / indicators.length : 0;
  const isAccident = averageConfidence > 0.4 && indicators.length >= 2;

  let summary = '';
  if (isAccident) {
    summary = `High probability accident scene detected with ${indicators.length} indicators. `;
    summary += indicators.map(i => i.type).join(', ') + ' identified.';
  } else if (indicators.length > 0) {
    summary = `Possible accident indicators detected but confidence is low. Further verification recommended.`;
  } else {
...
function detectVehicleDamage(imageData: ImageData): { detected: boolean; confidence: number } {
...
function detectDebris(imageData: ImageData): { detected: boolean; confidence: number } {
  const { data, width, height } = imageData;
...
function detectEmergencyVehicles(imageData: ImageData): { detected: boolean; confidence: number } {
  ...
  const confidence = Math.min((redBluePatterns / sampleSize) * 5 + (brightColors / sampleSize) * 2, 1);
...
function detectBrokenGlass(imageData: ImageData): { detected: boolean; confidence: number } {
  const { data } = imageData;
...
function detectSkidMarks(imageData: ImageData): { detected: boolean; confidence: number } {
